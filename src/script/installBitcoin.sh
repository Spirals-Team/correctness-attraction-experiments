...
files=(org/)
output=bitcoinj/core/src/main/java/
...
do
   echo spooning $i
   echo "java -classpath $lib:$bitcoinjjar:$jPerturb spoon.Launcher --with-imports -i $path/$i:$perturbation -o $output -p $processors"
   java -classpath $lib:$bitcoinjjar:$jPerturb spoon.Launcher --with-imports -i $path/$i:$perturbation -o $output -p $processors
done
