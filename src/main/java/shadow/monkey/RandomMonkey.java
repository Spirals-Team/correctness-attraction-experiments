...
public class RandomMonkey extends MonkeyImpl {
    ...
    private void clickOnLink() throws Exception {
        ...
        elements = elements.stream().filter(button ->
                button.isDisplayed() && button.isEnabled() &&
                        !button.getAttribute("class").startsWith("addthis")
    ...
    private boolean leave() {
        List<WebElement> elements = driver.findElements(By.tagName("a")).stream().filter(link ->
                link.getAttribute("class").startsWith("modalCloseImg") &&
                        !"hidden".equals(link.getAttribute("type"))
        ).collect(Collectors.toList());
