...
public class BitcoinOracle implements Oracle<Tuple,Integer> {
    ...
    public boolean assertPerturbation(Tuple input, Integer output) {
        ...
        int currentAmountSender = BitcoinManager.btcStringToBtcInt(this.manager.getWalletAppKit(input.get(0)).wallet().getBalance().toFriendlyString());
        int currentAmountReceiver = BitcoinManager.btcStringToBtcInt(this.manager.getWalletAppKit(input.get(1)).wallet().getBalance().toFriendlyString());
